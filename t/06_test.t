...
use constant APID_REPL    =>
  "some.ridicously.long.email.address.used.for.test.purposes\@test.it";

my $qt = new Audio::M4P::QuickTime( file => 't/emotre.m4a' );
isa_ok( $qt, 'Audio::M4P::QuickTime' );

my $ilst = $qt->FindAtom("ilst");
ok( $ilst->size == ILST_SIZE && $ilst->start == ILST_START,
    "ilst atom placement" );

my $cnid = $qt->FindAtom("cnID");
ok( $cnid->size == CNID_SIZE && $cnid->start == CNID_START,
    "cnID atom placement" );
...
ok( $apid->size == APID_SIZE && $apid->start == APID_START,
    "apID atom placement" );
...
ok( $cnid->size == CNID_SIZE && $cnid->start == ( CNID_START + $diffsz ),
    "new cnID atom placement" );

@cnIDContent = unpack( "Na4h16N", $cnid->data );
ok( $cnIDContent[3] == CNID_CONTENT, "same cnID content" );

$ilst = $qt->FindAtom("ilst");
ok( $ilst->size == ( ILST_SIZE + $diffsz ) && $ilst->start == ILST_START,
    "new ilst atom size" );
